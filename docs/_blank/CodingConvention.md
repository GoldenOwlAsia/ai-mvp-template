...
> - Rules here are mandatory — they must not be changed on your own
...
✅ Max file length: 300 lines — split the file if longer
✅ Max function length: 50 lines — split the function if longer
❌ Do not use `any`
❌ Do not use `// @ts-ignore`
❌ Do not leave unnecessary comments (commented-out code)
❌ Do not use magic numbers — name a constant instead
❌ Do not use magic strings — use an enum or constant instead
```
...
// Do not use the I prefix — use a clear name instead of IUser
// ❌ interface IUser {}
// ✅ interface User {}
...
// Do not export multiple components from one file (except small helpers)
```
...
// Decorator: camelCase for decorator functions
...
```typescript
// ✅ Explicit type — do not let TypeScript infer for a public API
function getUser(id: string): Promise<User> { ... }

// ✅ Use a type alias for union types
type TaskStatus = 'TODO' | 'IN_PROGRESS' | 'DONE'

// ✅ Use an interface for an object shape (extendable)
interface User {
  id: string
  email: string
}

// ✅ Optional chaining — do not check null manually
const name = user?.profile?.name ?? 'Unknown'

// ✅ Non-null assertion only when certain
const element = document.getElementById('root')!

// ✅ Readonly for an immutable object
interface Config {
...
// ❌ Do not use the Function type
type Handler = Function                    ❌
type Handler = (event: Event) => void      ✅

// ❌ Do not use the Object type
type Config = Object                       ❌
...
// All API calls go through the service layer — never call axios directly in a component
...
// ❌ Do not use inline styles (except for a dynamic value not available in Tailwind)
<div style={{ color: 'red' }}>     ❌
<div className="text-red-500">     ✅

// ❌ The only exception for inline style — a dynamic value
<div style={{ backgroundColor: project.color }}>   ✅ (color comes from the DB)
```
...
export class TaskService {
  ...
  async findAll(projectId: string, userId: string, query: QueryTaskDto) {
    ...
    // 4. Return (do not throw directly — let the filter handle it)
    return {
      data: tasks,
      meta: {
        total,
...
```typescript
// The controller only handles: routing, auth guards, input validation, response format
// No business logic
...
```typescript
// A DTO always has full validation decorators
// Always has a clear error message
...
// UpdateDto always uses PartialType — do not rewrite each field
...
```typescript
// Use NestJS built-in exceptions — do not throw a raw Error
...
// Always select only the fields needed for nested relations
include: {
  assignee: {
    select: { id: true, name: true, avatarUrl: true }
    // ❌ Do not select: password, isVerified, lastLoginAt...
  }
}
...
// Using polling instead of WebSocket to reduce complexity for v1
const REFRESH_INTERVAL = 30_000

// ✅ TODO comment with a ticket reference
// TODO: [TASK-123] Cache the result when data exceeds 1000 records

// ✅ JSDoc for public APIs
/**
 * Creates a new task in the project.
 * Automatically sends a notification to the assignee, if any.
...
// ❌ Comments that just explain obvious code (the code is self-documenting)
...
type:
  feat     — a new feature
  fix      — a bug fix
  chore    — no impact on logic (dependency updates, config...)
  refactor — refactor with no new feature and no bug fix
  test     — adding or fixing tests
  docs     — adding or fixing docs
  style    — formatting, spacing (no impact on logic)
...
// ✅ Move everything into the service
...
// ✅ Use early return or a clear if/else
...
// ❌ Leftover console.log used for debugging
console.log('user:', user)   ❌ — do not commit
// ✅ Use a logger service (backend) or remove it (frontend)
