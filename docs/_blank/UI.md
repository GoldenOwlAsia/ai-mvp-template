...
> Claude must use the correct library and not import anything extra on its own.
...
> Claude must use these tokens correctly — do not hard-code colors, spacing, or fonts.
...
> Use an existing component — do not create a new one if one already exists.
...
- Error message right below the input, text-red-500
...
**Rule:**
- Destructive actions always use `AlertDialog` (2-step confirmation), never `Dialog`
- Form dialog width: `sm:max-w-[480px]` for simple forms, `sm:max-w-[640px]` for complex forms
- Every dialog must have a Cancel button
...
- Must include: icon, title, description, CTA action
...
- API errors: toast (do not use alert)
- Always use toast.promise for async actions
- Error message from the API when available, fallback to "Something went wrong"
...
**Rule:**
- Destructive items always go at the bottom, separated by a separator
- Destructive items are red: `className="text-red-600 focus:text-red-600"`
...
> Complex components specific to each feature.
> Do not recreate anything already in this list.
...
| `ProjectForm` | `features/project/components/ProjectForm.tsx` | Form for creating and editing a project |
...
| `DateDisplay` | `components/common/DateDisplay.tsx` | Format + relative date |
...
> Claude must follow all of these rules — do not skip them for the sake of "being faster".
...
✅ Color must never be the only way information is conveyed
   (a status badge should have both icon/text, not just color)
❌ Do not use a div with onClick instead of a button
❌ Do not omit alt text for img
```
...
```typescript
// Transition — only for interactive elements
...
// Do not animate layout shift (avoid CLS)
// Do not use animation on the critical path
```
